...
#define ZOOM_FACTOR  1.667 /* For zooming on the graphics */
#define ZOOM_FIT_FACTOR 2.0
#endif
...
// Contains all coordinates useful for graphics transformation
 ...
 t_transform_coordinates obtainTranscoord() {
     return trans_coord;
 }
...
//Zoom fit to a specific intersection
void zoom_fit(void (*drawscreen)(void), float x, float y, bool zoom_fit) {       
    handle_zoom_fit(x, y, drawscreen, zoom_fit);
}
...
void zoom_out(void (*drawscreen) (void)) {
    float xcen, ycen;
    std::cout << trans_coord.top_width << std::endl;
    xcen = (trans_coord.xright + trans_coord.xleft) / 2;
    ycen = (trans_coord.ybot + trans_coord.ytop) / 2;
...
void handle_zoom_fit(float x, float y, void (*drawscreen) (void), bool zoom_fit) {
    
     if (zoom_fit == false) {
        //make xright - xleft = 0.6 of the original distance
         
        trans_coord.xleft = x - 0.01;
        trans_coord.xright = x + 0.01;
        //make ybot - ytop = 0.6 of the original distance
        trans_coord.ytop = y + 0.01;
        trans_coord.ybot = y - 0.01;
     }
        update_transform();
        if (drawscreen != NULL && zoom_fit == false) {
            drawscreen();
        }
}
...
void handle_zoom_in(float x, float y, void (*drawscreen) (void)) {
    ...
    float scale_lat1, scale_lat2, scale_lon1, scale_lon2;
    
    scale_lon1 = xscrn_to_world(1120)/cos(lat_avg);
    scale_lat1 = 870;
    scale_lon2 = xscrn_to_world(1120+3000/LON_TO_DIS*cos(lat_avg))/cos(lat_avg);
    scale_lat2 = 870;
    LatLon point1(scale_lat1, scale_lon1);
    LatLon point2(scale_lat2, scale_lon2);
...
//                printf("x: %d    y: %d\n", report.xmotion.x, report.xmotion.y);
        ...
        // Extra if statement to see if proceed button was pressed
        if (gl_state.ProceedPressed) {
            x11_turn_on_off(OFF);
            flushinput();
            gl_state.ProceedPressed = false;
            return;
...
static void x11_build_textarea(void) {
    ...
    x11_state.textarea = XCreateSimpleWindow(x11_state.display, x11_state.toplevel, 0,
        ...
        T_AREA_HEIGHT, 0, x11_convert_to_xcolor(predef_colors[WHITE]), 
        x11_convert_to_xcolor(predef_colors[WHITE]));
...
// 	}
// }
std::string draw_box::getinput() {
    return input;
}

void draw_box::updateinput(std::string inp) {
    input=inp;
}

void draw_box::draw(bool searchbar_clicked) {
    ...
    if (StreetData::clear == true)
        return;
    else if (getinput().size() <= 31 && StreetData::clear ==false)
        drawtext(xscrn_to_world(obtainTranscoord().top_width/2),yscrn_to_world(obtainTranscoord().top_height/10), getinput() , 1, 1);
    else if (StreetData::clear ==false)
        drawtext(xscrn_to_world(obtainTranscoord().top_width/2),yscrn_to_world(obtainTranscoord().top_height/10), getinput().substr(getinput().size()-30, 30) , 1, 1);
